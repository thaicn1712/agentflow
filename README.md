...
`agentflow` is that stack for Rust, in one crate: five independent crates, each solving a gap Python's ecosystem has and Rust's didn't, composed into one AgentOps lifecycle.
...
Each is a standalone crate you can `cargo add` on its own. `agentflow` re-exports all five under one dependency, plus glue code for the parts of the lifecycle that genuinely benefit from knowing about each other.
...
Re-exports — use any of the five exactly as documented in their own READMEs:

```rust,ignore
use agentflow::{stream, guard, act_guard, schema, eval};
...
## Why one crate instead of five

Each crate is deliberately independent — no crate requires another, so you're never forced into the whole stack. `agentflow` exists for the case where you want all five without hunting down five separate `cargo add`s, and for the parts of the lifecycle (guard + evaluate together, in this v0.1) where composing them by hand would just be boilerplate.
